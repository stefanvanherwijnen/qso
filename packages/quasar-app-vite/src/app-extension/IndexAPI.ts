...
export default class IndexAPI {
  ...
  constructor(
    { extId, ctx, appPaths }:
      { extId: string, ctx: Record<string, any>, appPaths: AppPaths }) {
    this.ctx = ctx
    this.extId = extId
    // this.prompts = prompts
    this.resolve = appPaths.resolve
    this.appDir = appPaths.appDir
    this.extensionJson = new ExtensionJson(appPaths)
   ...
   */
  getPersistentConf () {
    return this.extensionJson.getInternal(this.extId)
  }
  ...
  setPersistentConf (cfg: Record<string, any>) {
    this.extensionJson.setInternal(this.extId, cfg || {})
  }
  ...
  async compatibleWith (packageName: string, semverCondition: string) {
    // const json = await getPackageJson(packageName)

    // if (!json) {
    //   return fatal(`Extension(${this.extId}): Dependency not found - ${packageName}. Please install it.`)
    // }
  ...
  hasExtension (extId: string) {
    return this.extensionJson.has(extId)
  }
