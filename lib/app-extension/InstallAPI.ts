...
import getCallerPath from '@stefanvh/quasar-app-vite/lib/helpers/get-caller-path'
import extensionJson from '@stefanvh/quasar-app-vite/lib/app-extension/extension-json'
import { warn, fatal } from '@stefanvh/quasar-app-vite/lib/helpers/logger'
...
export default class InstallAPI {
  ...
  resolve: typeof appPaths.resolve
  appDir: string
  __needsNodeModulesUpdate: boolean
  __hooks: Record<string, any>
  constructor (
    { extId, prompts }:
    { extId: string, prompts: Record<string, any>[] }) {
    this.extId = extId
  ...
  mergePersistentConf (cfg = {}) {
    ...
    this.setPersistentConf(Object.assign({}, currentCfg, cfg))
  }
   ...
   */
  hasPackage (packageName: string, semverCondition: string) {
    // const json = getPackageJson(packageName)
   ...
   */
  async extendPackageJson (extPkg: Record<string, any> | string) {
    if (!extPkg) {
      return
    }
    ...
    const pkg = Object.assign({}, await import(filePath), extPkg)
    ...
    if (typeof extPkg === 'string') return
   ...
   */
  async extendJsonFile (file: string, newData: Record<string, any>) {
    if (newData !== void 0 && Object(newData) === newData && Object.keys(newData).length > 0) {
      ...
      try {
        const data = Object.assign({}, fs.existsSync(filePath) ? await import(filePath) : {}, newData)
   ...
   */
  render (templatePath: string, scope: Record<string, any>) {
    const dir = getCallerPath()
   ...
   * @param {object} scope (optional; rendering scope variables)
   */
  renderFile (relativeSourcePath: string, relativeTargetPath: string, scope: Record<string, any>) {
    const dir = getCallerPath()
